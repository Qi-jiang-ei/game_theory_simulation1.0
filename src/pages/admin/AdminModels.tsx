...
import { Trash2, Edit, Plus, Eye, X } from 'lucide-react'; // 在这里导入 X 图标组件
...
export const AdminModels: React.FC = () => {
  ...
  const fetchModels = async () => {
    const data = await fetchData('game_models', '*');
    // 检查 data 是否为 GameModelRecord 类型的数组
    if (Array.isArray(data) && data.every(item => 'id' in item && 'name' in item && 'type' in item && 'description' in item && 'config' in item && 'created_at' in item)) {
      setModels(data as GameModelRecord[]);
    } else {
      console.error('获取的数据格式不正确，无法更新模型列表。', data);
    }
    setLoading(false);
  };
